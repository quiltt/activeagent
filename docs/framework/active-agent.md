...
Active Agent provides class methods like `Agent.prompt(...)` and `Agent.embed(...)` for testing and quick prototyping, as well as instance action methods for production applications that can be used by any agent that inherits from `ActiveAgent::Base`.

For production use, **define custom action methods** in your agent classes to organize agent behaviors. Both actions and class methods can optionally use view templates for complex formatting.
...
- **[Action methods](/actions/actions)**: Define public methods that become callable tools or functions for the Agent to perform actions that can render prompts to the agent or generative views to the user.
- **[Queued Generation](/agents/queued-generation)**: Manage asynchronous prompt generation and response cycles with Active Job, allowing for efficient processing of requests.
- **[Callbacks](/agents/callbacks)**: Use `before_action`, `after_action`, `before_generation`, `after_generation` callbacks to manage prompt context and handle generated responses.
- **[Streaming](/agents/callbacks#on-stream-callbacks)**: Support real-time updates with the `on_stream` callback to the user interface based on agent interactions.
