  ...
  module GenerationProvider
    class OpenRouterProvider < OpenAIProvider
      ...
      def initialize(options)
        @track_costs = options.delete("track_costs") != false
        super
      end
      ...
      def generate(prompt)
        ...
        with_error_handling do
          parameters = @options.chat_parameters # Provider Level
          parameters.merge!(prompt_parameters) # Prompt Level
      ...
      def supports_vision?(model = @options.model)
        VISION_MODELS.include?(model)
      end

      def supports_structured_output?(model = @options.model)
      ...
      def add_openrouter_params(params)
        ...
        if @provider_preferences.present? || @data_collection != "allow" || runtime_data_collection
          params[:provider] = nil #build_provider_preferences
        end
